...
const Detail = ({ value, title }) => {
  return (
    <div className='mb4'>
      <Title>{title}</Title>
      <div className='bg-white pa2 br2 truncate monospace' data-test={value}>{value}</div>
    </div>
  )
}
...
const Title = ({ children }) => {
  return (
    <h2 className='f5 ma0 pb2 tracked aqua fw4'>{children}</h2>
  )
}
...
const App = () => {
  ...
  const [textContent, setTextContent] = useState('hello world from webpacked IPFS')
  ...
  return (
    <div className="montserrat">
      ...
      <main className="pa4-l bg-snow mw7 mv5 center pa4">
        ...
        {error &&
          <span className="f7 red db mt3">{error}</span>
        }
      </main>
    </div>
  )
