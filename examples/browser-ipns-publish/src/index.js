...
async function main () {
  ...
  const log = Logger(document.getElementById('console'))
  const sLog = Logger(document.getElementById('server-console'))
  let keyName = 'self'
  ...
  async function wsConnect (addr) {
    ...
    if (!ipfsBrowser) { throw new Error('Wait for the local IPFS node to start first') }
  ...
  async function publish (content) {
    ...
    if (keyName != 'self') {
      ...
      const r = await ipfsBrowser.key.list()
    ...
    if (!remList.includes(topic)) { sLog(`<span class="red">[Fail] !Pubsub.ls ${topic}</span>`) } else sLog('[Pass] Pubsub.ls')

    const remListSubs = await ipfsAPI.name.pubsub.subs() // API
    if (!remListSubs.includes(`/ipns/${keys.id}`)) { sLog(`<span class="red">[Fail] !Name.Pubsub.subs ${keys.id}</span>`) } else sLog('[Pass] Name.Pubsub.subs')
